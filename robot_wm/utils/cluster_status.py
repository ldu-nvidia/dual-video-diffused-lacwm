...
parser.add_argument(
    "--account", type=str, required=True, help="Account name for SLURM query"
)
...
def get_qos_for_account(account):
    ...
    result_qos = subprocess.run(
        sacctmgr_qos_command, shell=True, capture_output=True, text=True
    )
    ...
    for line in output.splitlines()[1:]:  # Skip the header
        fields = line.split("|")
        if (
            len(fields) == 3 and fields[2].strip()
        ):  # Ensure we have 3 fields and QOS is not empty
...
for qos_name in qos_list:
    ...
    sacctmgr_command = ["sacctmgr", "show", "qos", qos_name, "format=GrpTRES%50", "-P"]

    # Fetch total allowable resources for the QOS
    result_qos_resources = subprocess.run(
        sacctmgr_command, capture_output=True, text=True
    )
    ...
    for job in job_data:
        ...
        if "." not in job_id and user and state == "RUNNING":
            ...
            mem_usage_per_user[user] += int(
                mem_usage
            )  # rounding memory usage to integer
    ...
    table = Table(
        title=f"Resource Usage Summary per User (QOS: {qos_name})", box=box.SQUARE
    )
    ...
    table.add_row(
        "Grand Total",
        f"{grand_total_gpu_usage:,}",
        f"{grand_total_cpu_usage:,}",
        f"{grand_total_mem_usage:,}",
        style="bold",
    )
    ...
    formatted_mem_limit = (
        f"{int(memory_limit):,}" if memory_limit != "N/A" else memory_limit
    )

    # Add a row for the total allowable resources for the specific QOS
    table.add_row(
        f"Total Allowable Resources (QOS: {qos_name})",
        formatted_gpu_limit,
        formatted_cpu_limit,
        formatted_mem_limit,
        style="bold cyan",
