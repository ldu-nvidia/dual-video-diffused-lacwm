"""Guarded Trainer specialization for the paired LaMo continuation screen."""
...
class LamoMotionDriftTrainer(Trainer):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        if not load_path.is_absolute() or not load_path.is_file() or load_path.is_symlink():
            raise RuntimeError("LaMo continuation requires the regular absolute VPM snapshot")
        expected_sha = os.environ.get("LAMO_VPM_SNAPSHOT_SHA256")
        if expected_sha != PARENT_SNAPSHOT_SHA256:
            raise RuntimeError("LAMO_VPM_SNAPSHOT_SHA256 is absent or differs")
        observed_sha = (
            _sha256(load_path)
        ...
        if list(config.get("exclude_keys", [])):
            raise RuntimeError("LaMo continuation must strictly load every VPM key")
        ...
        if (
            ...
            raise RuntimeError(
                "LaMo arms require a fresh optimizer/model-only continuation"
            )
        if (
        ...
        weight = float(getattr(module, "motion_drift_weight", float("nan")))
        if weight not in {0.0, 0.4}:
            raise RuntimeError("screen arm must use lambda 0.0 or 0.4")
        self._lamo_arm = "VPM-CONT" if weight == 0.0 else "VPM-DRIFT"
        self._lamo_trace_path = self.save_path.parent / "paired_training_trace.jsonl"
        self._lamo_trace_complete = self.save_path.parent / "paired_training_trace_complete.json"
        if self.is_main_process:
            if self._lamo_trace_path.exists() or self._lamo_trace_complete.exists():
                raise RuntimeError("fresh paired-training trace output already exists")
            header = {
                "kind": "lamo_motion_drift_training_trace_header",
                "arm": self._lamo_arm,
                "motion_drift_weight": weight,
    ...
    def _log(self, metrics: dict[str, Any]) -> None:
        if self.is_main_process:
            ...
            with self._lamo_trace_path.open("ab") as handle:
                handle.write(
    ...
    def train(self):
        ...
        if result == "completed" and self.is_main_process:
            rows = sum(1 for _ in self._lamo_trace_path.open("rb"))
            _exclusive_json(
                self._lamo_trace_complete,
                {
                    "kind": "lamo_motion_drift_training_trace_complete",
                    "arm": self._lamo_arm,
                    "rows": rows,
                    "trace_sha256": _sha256(self._lamo_trace_path),
