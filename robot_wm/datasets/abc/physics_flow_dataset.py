...
class ABCPhysicsFlowDataset(ABCVideoResidualAnchorDataset):
    ...
    def _validate_flow_metadata(self) -> str:
        ...
        if (
            meta.get("schema") != "physics-flow-cache-v1"
            ...
            or meta.get("flow_shape") != list(self._flow_shape)
            ...
            != self.expected_renderer_analysis_identity_sha256
            or meta.get("renderer_decision") != "GO_FOR_WAN_SCREEN"
    ...
    def _open_flows(self) -> np.ndarray:
        if self._flows is None:
            ...
            if self._flows.shape != self._flow_shape or self._flows.dtype != np.float16:
    ...
    def _validate_flow_array(self) -> None:
        ...
        for index in sorted({0, self.expected_clip_count - 1}):
            flow = torch.from_numpy(np.array(values[index : index + 1], copy=True)).float()
            validate_packed_flow(flow)
        self._flows = None

    def _get_sample(self, index: int) -> dict[str, torch.Tensor]:
        sample = super()._get_sample(index)
        flow = torch.from_numpy(np.array(self._open_flows()[int(index)], copy=True)).float()
