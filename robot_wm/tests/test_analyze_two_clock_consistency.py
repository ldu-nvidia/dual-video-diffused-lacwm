"""Tests for the fixed paired two-clock consistency validation decision rule."""
...
from tools import analyze_two_clock_consistency as analysis
...
def test_paired_effect_sign_is_positive_when_candidate_arm_is_lower():
    ...
    assert (
        result["one_sided_simultaneous_lower_bound"]["family_contrast_count"]
        == 1
    )
...
def test_cross_arm_pairing_detects_noise_mismatch():
    ...
    with pytest.raises(analysis.TwoClockConsistencyAnalysisError, match="pairing differs"):
    ...
    *,
    ...
    with path.open("x", encoding="utf-8") as handle:
        ...
        for update in range(200):
            ...
            metrics["train_loss/paired_audit/sigma_hi_mean"] = 0.9
            metrics["train_loss/paired_audit/sigma_lo_mean"] = 0.2
            metrics["train_loss/two_clock_consistency/model_calls"] = 2.0
            metrics[
...
def test_training_pair_requires_all_200_exact_audit_updates(tmp_path):
    candidate = _write_trace(tmp_path / "candidate.jsonl", arm="TC-CONS")
    reference = _write_trace(tmp_path / "reference.jsonl", arm="TC-CONT")
    assert analysis._assert_training_pair(candidate, reference)["updates"] == 200

    mismatched = _write_trace(
        tmp_path / "mismatch.jsonl", arm="TC-CONS", mismatch_update=37
    )
    with pytest.raises(analysis.TwoClockConsistencyAnalysisError, match="update 37"):
