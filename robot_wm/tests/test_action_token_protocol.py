"""Protocol tests for action-token evaluation and fixed decision gates."""
...
from tools.analyze_action_token import _effect_gate, paired_interaction_effect
...
def _row(index: int, endpoint: screen.Endpoint, arm: screen.Arm) -> dict:
    ...
    donor = (
        ...
        if source == "episode_shuffled"
        else None
    )
    token_active = arm.action_token_enabled and endpoint.token_mode == "native"
    context_label = f"context:{index}:{source}" if token_active else "null-context"
    context_delta_max = 0.1 if token_active else 0.0
    context_token_rms = (
        [0.1 + 0.001 * offset for offset in range(40)]
        if token_active
        else [0.0] * 40
    )
    hashes = {
        ...
        "raw_action_context_tokens_sha256": _hash(
            f"raw-context:{index}:{source}"
        ),
        "wan_context_sha256": _hash(context_label),
        "action_context_delta_sha256": _hash(context_label + ":delta"),
    ...
    return screen.identity_payload(
        {
            ...
            "action_context_delta_rms": context_delta_max,
            "action_context_delta_abs_max": context_delta_max,
            "action_context_token_rms": context_token_rms,
...
def test_endpoint_grid_has_aligned_quality_and_zero_global_shuffle_attribution():
    assert [endpoint.code for endpoint in screen.ENDPOINTS] == [
        ...
        "aligned_nfe_4",
        "zero_nfe_1",
        "episode_shuffled_nfe_1",
        "aligned_tokens_masked_nfe_1",
    ]
...
def test_rows_require_exact_calls_no_future_inputs_and_episode_disjoint_donor():
    ...
    with pytest.raises(evaluation.ActionTokenEvaluationError, match="protocol"):
        evaluation._validate_rows(rows, arm, _registration())


def test_rows_reject_changed_paired_noise():
    arm = screen.ARM_BY_CODE["AT-ON"]
    rows = _rows(arm)
    changed = dict(rows[1])
    ...
    with pytest.raises(evaluation.ActionTokenEvaluationError, match="noise"):
        evaluation._validate_rows(rows, arm, _registration())
