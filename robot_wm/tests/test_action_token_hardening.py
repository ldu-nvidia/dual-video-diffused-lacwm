"""Regression tests for the adversarially hardened action-token screen."""
...
from lam.action_token_conditioned_model import (
    ActionTokenConditionedVPM,
    ActionTokenContractError,
)
from robot_wm.utils.action_token_trainer import (
    _load_parent_state_exact,
    _state_sha256,
)
from tools import action_token_evaluate as evaluation
...
def test_registration_recomputes_stats_instead_of_trusting_source_label(tmp_path):
    ...
    fitted = screen._computed_action_token_stats(actions_path)
    stats = {
        **fitted,
        "source": {
    ...
    forged["mean"][0] += 0.25
    with pytest.raises(screen.ActionTokenScreenError, match="recomputed"):
...
class _ParentPlusResidual(nn.Module):
    def __init__(self) -> None:
        ...
        self.action_token_adapter = nn.Linear(2, 2)
...
def test_action_screen_uses_only_an_inert_auxiliary_placeholder():
    ...
    inert = ActionTokenConditionedVPM._resolve_auxiliary_clean(
    ...
    with pytest.raises(ActionTokenContractError, match="forbids cached"):
        ActionTokenConditionedVPM._resolve_auxiliary_clean(
            model,
            rgb,
            latent_shape,
...
def test_context_injection_uses_native_null_sequence_and_exact_hard_off():
    model = object.__new__(ActionTokenConditionedVPM)
    nn.Module.__init__(model)
    model.register_buffer("null_prompt", torch.randn(1, 4096), persistent=False)
    model.forward_model = SimpleNamespace(transformer=SimpleNamespace(text_len=512))
    model.action_token_adapter = _GateOnly(0.0)
    model._current_action_context_tokens = torch.randn(2, 40, 4096)
    model._capture_action_token_audit = False
    model.paired_audit_exact = {}
    model.action_token_telemetry = {}

    off = torch.stack(model._build_context(2, torch.device("cpu"), torch.float32))
    padded = torch.cat(
        (model.null_prompt, torch.zeros(511, 4096)), dim=0
    )
    expected = padded.unsqueeze(0).expand(2, -1, -1)
    assert torch.equal(off, expected)

    model.action_token_adapter.gate.fill_(0.25)
    on = torch.stack(model._build_context(2, torch.device("cpu"), torch.float32))
    assert torch.equal(on[:, :-40], expected[:, :-40])
    assert torch.equal(
        on[:, -40:],
        expected[:, -40:] + 0.25 * model._current_action_context_tokens,
    )
...
def test_parent_load_misses_exactly_residual_and_rejects_any_other_missing_key():
    ...
    parent = {
        ...
        if not key.startswith("action_token_adapter.")
    }
    new_keys = {
        key for key in target.state_dict() if key.startswith("action_token_adapter.")
    }
...
def test_guarded_launcher_sets_registered_pythonpath_and_batch_one():
    ...
    sbatch = (root / "tools/slurm/action_token.sbatch").read_text()
    submit = (root / "tools/slurm/submit_action_token.sh").read_text()
    workflow = (root / "tools/slurm/action_token_workflow.py").read_text()
    entrypoint = (
        root / "projects/latent_action_models/train_action_token.py"
    ).read_text()
    trainer = (root / "robot_wm/utils/action_token_trainer.py").read_text()
    evaluator = (root / "tools/action_token_evaluate.py").read_text()
    analyzer = (root / "tools/analyze_action_token.py").read_text()
...
def _runtime_registration(tmp_path: Path) -> dict:
    ...
    return {
        ...
        "action_token_stats": {"file": {**record, "sha256": "c" * 64}},
        "runtime": {
...
def test_registered_input_and_runtime_receipts_are_exact(tmp_path, monkeypatch):
    ...
    with pytest.raises(screen.ActionTokenScreenError, match="revalidation differs"):
    ...
    with pytest.raises(screen.ActionTokenScreenError, match="different Slurm"):
...
def test_arm_plan_uses_the_complete_canonical_endpoint_schema(tmp_path):
    ...
    plan_core = {
        "schema_version": 1,
        "kind": "action_token_arm_execution_plan",
    ...
    assert all("token_mode" in endpoint for endpoint in screen.endpoint_records())

    forged = dict(plan_core)
    forged["evaluation"] = dict(plan_core["evaluation"])
    forged["evaluation"]["endpoints"] = [
        {key: value for key, value in endpoint.items() if key != "token_mode"}
        for endpoint in screen.endpoint_records()
    ]
    plan_path.write_text(json.dumps(screen.identity_payload(forged)))
    with pytest.raises(evaluation.ActionTokenEvaluationError, match="plan differs"):
...
def test_wandb_completion_receipt_is_semantically_bound(tmp_path):
    ...
    registration = {
        ...
        "action_token_stats": {"file": {"sha256": "c" * 64}},
    }
    ...
    core = {
        "schema_version": 1,
        "kind": "action_token_wandb_run_complete",
        "registration_identity_sha256": registration["identity_sha256"],
        "arm": {
            "code": arm.code,
    ...
    with pytest.raises(
        evaluation.ActionTokenEvaluationError,
