"""Explicit-action Wan model with a training-only low-frequency motion loss.
...
differs.  At training time ``x0_hat = x_sigma - sigma*v_hat`` and the clean
future are spatially low-passed per camera view, then compared through their
adjacent temporal differences.  The last observed Wan token supplies a causal
anchor, so the two-token future yields two rather than one transitions.  The
...
from robot_wm.modeling.dual_diffusion.low_frequency_motion import (
    low_frequency_motion_consistency,
)


class LowFrequencyMotionTrainingOnlyContractError(RuntimeError):
...
class LowFrequencyMotionTrainingOnlyExplicitActionDiT(ExplicitActionDiTModel):
    """Parameter-identical OFF/ON model; motion loss exists only in training."""

    def __init__(self, *, motion_band_regularization: Mapping[str, Any], **kwargs: Any):
        config = dict(motion_band_regularization)
        self.motion_band_loss_weight = float(config.get("loss_weight", 0.0))
        self.motion_band_num_views = int(config.get("num_views", 3))
        self.motion_band_kernel_size = int(config.get("kernel_size", 5))
        self.motion_band_sigma = float(config.get("sigma", 1.0))
        self.motion_band_beta = float(config.get("beta", 0.25))
        self.motion_band_epsilon = float(config.get("epsilon", 1.0e-4))
        if self.motion_band_loss_weight < 0:
            raise ValueError("motion loss_weight must be non-negative")
        if self.motion_band_num_views != 3:
            raise ValueError("controlled screen requires exactly three camera views")
        if self.motion_band_kernel_size != 5 or self.motion_band_sigma != 1.0:
            raise ValueError("controlled screen fixes the Gaussian to 5x5, sigma=1")
        if self.motion_band_beta <= 0 or self.motion_band_epsilon <= 0:
            raise ValueError("motion beta/epsilon must be positive")
        ...
        if any(hasattr(self.forward_model, name) for name in forbidden):
            raise LowFrequencyMotionTrainingOnlyContractError(
                "training-only motion supervision must not instantiate a TF branch"
            )
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if (
            latent_frames != 4
            or history_frames != 2
            or tuple(clean.shape[-2:]) != (24, 120)
        ):
            raise LowFrequencyMotionTrainingOnlyContractError(
                "screen requires [B,16,4,24,120] Wan geometry "
                "(2 history + 2 future tokens)"
            )
        ...
        motion_band_total = flow_loss.new_zeros(())
        anchor_transition = flow_loss.new_zeros(())
        future_transition = flow_loss.new_zeros(())
        target_rms_mean = flow_loss.new_zeros(())
        valid_transition_count = 0
        if self.motion_band_loss_weight > 0:
            terms = low_frequency_motion_consistency(
                predicted_clean[:, :, history_frames:],
                clean[:, :, history_frames:],
                clean[:, :, history_frames - 1 : history_frames],
                validity_mask=loss_mask[:, :, history_frames - 1 :],
                num_views=self.motion_band_num_views,
                kernel_size=self.motion_band_kernel_size,
                sigma=self.motion_band_sigma,
                beta=self.motion_band_beta,
                epsilon=self.motion_band_epsilon,
            )
            motion_band_total = terms.total
            anchor_transition = terms.anchor_transition
            future_transition = terms.future_transition
            target_rms_mean = terms.target_rms_mean
            valid_transition_count = terms.valid_transition_count
        total = flow_loss + self.motion_band_loss_weight * motion_band_total
        ...
        self.aux_losses.update(
            {
                ...
                "motion_band/total": motion_band_total.detach(),
                "motion_band/anchor_transition": anchor_transition.detach(),
                "motion_band/future_transition": future_transition.detach(),
                "motion_band/target_rms_mean": target_rms_mean.detach(),
                "motion_band/loss_weight": flow_loss.new_tensor(self.motion_band_loss_weight),
                "motion_band/valid_transition_count": flow_loss.new_tensor(
                    float(valid_transition_count)
                ),
                "state/predicted_clean_future_nmse": clean_nmse.detach(),
            }
        )
        if not bool(torch.isfinite(total)):
            raise FloatingPointError("training-only motion objective is non-finite")
        return total
    ...
    def sample_future_deployable(
        ...
        *,
        ...
        if (
            int(history_latent.shape[2]) != self.num_history_latent
            or tuple(history_latent.shape[-2:]) != (24, 120)
        ):
            raise LowFrequencyMotionTrainingOnlyContractError(
                "history-only VAE geometry changed"
            )
        ...
        if calls != nfe:
            raise LowFrequencyMotionTrainingOnlyContractError("sampler Wan call count differs from NFE")
