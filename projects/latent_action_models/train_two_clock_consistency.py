"""Dedicated Hydra entrypoint for the paired two-clock-consistency screen."""
...
from robot_wm.utils.two_clock_consistency_trainer import TwoClockConsistencyTrainer
...
def _setup(cfg: DictConfig) -> TwoClockConsistencyTrainer:
    dist.init_process_group()
    _seed_all(int(cfg.seed))
    trainer = None
    try:
        trainer = hydra.utils.instantiate(cfg.trainer)
        if not isinstance(trainer, TwoClockConsistencyTrainer):
            raise RuntimeError(
                "two-clock consistency entrypoint requires "
                "TwoClockConsistencyTrainer"
            )
        trainer.initialize_wandb(cfg)
        # W&B/model construction cannot shift the paired corruption stream.
...
def _teardown(trainer: TwoClockConsistencyTrainer) -> None:
    failures = []
