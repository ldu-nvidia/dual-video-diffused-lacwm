...
RAW_RFFT_CANDIDATE = {
    ...
    "status": "candidate_pending_terminal_provenance_compatibility",
    "accepted_as_screen_input": False,
}

# Array order is part of the immutable experimental contract.
ARMS: tuple[dict[str, Any], ...] = (
    {
        "name": "parseval_off_s000",
            ...
            "raw_rfft_comparison": {
                **RAW_RFFT_CANDIDATE,
                "compatibility_requirements": [
                    "all raw-RFFT arms terminal with successful immutable outcomes",
                    "matching warm-start SHA-256 and ABC manifest SHA-256",
                    "matching seed, optimizer updates, schedule, and NFE contract",
                    "matching metric implementation and evaluation sample identities",
                    "sampler semantics audited before cross-screen use",
                ],
                "policy": (
                    "This candidate is not an input to submission or arm success. "
                    "Cross-screen raw-RFFT comparisons are forbidden until a "
                    "separate compatibility audit changes the status."
                ),
            },
...
def _assert_resolved_contract(
    ...
    *,
    ...
    expected = {
        ...
        "model.time_frequency_transform.representation": arm["representation"],
        "wandb.enabled": True,
        "wandb.mode": "online",
    ...
    if problems:
        raise RuntimeError(
            "resolved representation-screen configuration violates its contract: "
            + "; ".join(problems)
        )
...
def _validate_screen_manifest(
    ...
    *,
    ...
    raw_candidate = payload.get("raw_rfft_comparison", {})
    for key, wanted in RAW_RFFT_CANDIDATE.items():
        if raw_candidate.get(key) != wanted:
...
def command_prepare_arm(args: argparse.Namespace) -> int:
    ...
    payload = pilot._identity_payload(
        {
            "schema_version": 1,
            "kind": "dual_abc_tf_representation_screen_arm",
            "created_at_utc": pilot._now(),
            "screen_id": screen_id,
            "screen_manifest": {
...
def command_record_submission(args: argparse.Namespace) -> int:
    ...
    if screen_payload.get("kind") != "dual_abc_tf_representation_screen":
...
def command_record_outcome(args: argparse.Namespace) -> int:
    ...
    if arm_payload.get("kind") != "dual_abc_tf_representation_screen_arm":
    ...
    payload: dict[str, Any] = {
        "schema_version": 1,
        "kind": "dual_abc_tf_representation_screen_arm_outcome",
        "created_at_utc": pilot._now(),
        "manifest": {
            "path": str(manifest),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    screen_parser = subparsers.add_parser(
        "create-screen", help="write the immutable representation-screen manifest"
    )
    for name in (
        "screen-id",
        "git-commit",
