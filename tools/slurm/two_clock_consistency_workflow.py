#!/usr/bin/env python3
"""Fail-closed workflow helpers for the two-arm two-clock consistency VPM screen.
...
from tools import analyze_two_clock_consistency as analysis  # noqa: E402
from tools import dual_abc_pilot  # noqa: E402
from tools import two_clock_consistency_evaluate as evaluation  # noqa: E402


SCHEMA_VERSION = 1
KIND_ARM_PLAN = "two_clock_consistency_arm_execution_plan"


class TwoClockConsistencyWorkflowError(RuntimeError):
...
def _registration(path: Path) -> dict[str, Any]:
    ...
    if observed != source:
        raise TwoClockConsistencyWorkflowError("tool source changed after registration")
    return value
...
def _arm(code: str) -> evaluation.Arm:
    ...
    if arm is None:
        raise TwoClockConsistencyWorkflowError(f"unknown arm: {code}")
    return arm
...
def arm_values(registration: dict[str, Any], arm: evaluation.Arm) -> dict[str, Any]:
    ...
    return {
        ...
        "wandb_run_id": arm_identity(registration, arm),
...
def _assert_fresh_arm(values: dict[str, Any]) -> None:
    for key in ("run_dir", "evaluation_dir"):
        ...
        if path.exists() or path.is_symlink():
            raise TwoClockConsistencyWorkflowError(f"fresh arm output exists: {path}")
...
def command_arm_values(args: argparse.Namespace) -> int:
    ...
    if args.format == "json":
        ...
        if any("\t" in field or "\n" in field for field in fields):
            raise TwoClockConsistencyWorkflowError("arm value contains a shell delimiter")
        print("\t".join(fields))
    return 0
...
def command_wandb_check(_args: argparse.Namespace) -> int:
    ...
    if result.get("access") != "PRIVATE" or result.get("viewer_username") != "zijiandu":
        raise TwoClockConsistencyWorkflowError("W&B personal project privacy check failed")
...
def _train_command(values: dict[str, Any], repo: Path) -> list[str]:
    return [
        ...
        str(
            ...
            / "train_two_clock_consistency.py"
        ),
        ...
        "wandb.group=null",
        f"+wandb.id={values['wandb_run_id']}",
        "+wandb.resume=never",
    ]
...
def _evaluation_command(
    ...
    return [
        ...
        "--standalone",
        "--nproc_per_node=8",
        str(repo / "tools" / "two_clock_consistency_evaluate.py"),
        "evaluate",
        "--registration",
        str(registration),
        "--arm",
...
def command_plan(args: argparse.Namespace) -> int:
    ...
    payload = {
        "schema_version": SCHEMA_VERSION,
        "kind": "two_clock_consistency_dry_run",
        "mode": "no_commands_executed_no_files_created",
        "registration_identity_sha256": registration["identity_sha256"],
        "arms": arms,
        "analysis": {
            "baseline_inventory": str(
                root / "evaluation" / "tc-cont" / "inventory.json"
            ),
            "candidate_inventory": str(
                root / "evaluation" / "tc-cons" / "inventory.json"
            ),
