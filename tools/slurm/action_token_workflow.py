#!/usr/bin/env python3
"""Render immutable commands for the paired action-token screen.
...
from tools import analyze_action_token as analysis  # noqa: E402
from tools import dual_abc_pilot  # noqa: E402
from tools import two_clock_consistency_evaluate as base  # noqa: E402

KIND_PLAN = "action_token_arm_execution_plan"


class ActionTokenWorkflowError(RuntimeError):
...
def _registration(path: Path) -> dict[str, Any]:
    ...
    if base._clean_source(Path(source["path"]), source["git_commit"], "tool") != source:
        raise ActionTokenWorkflowError("tool source changed after registration")
...
def _arm(code: str) -> screen.Arm:
    ...
    if arm is None:
        raise ActionTokenWorkflowError("unknown arm")
    return arm
...
def arm_values(registration: dict[str, Any], arm: screen.Arm) -> dict[str, str]:
    ...
    return {
        ...
        "stats": registration["action_token_stats"]["file"]["path"],
...
def _train_argv(values: dict[str, str], repo: Path) -> list[str]:
    return [
        ...
        "--standalone",
        "--nproc_per_node=8",
        str(repo / "projects" / "latent_action_models" / "train_action_token.py"),
...
def _config_contract(
    ...
    try:
        ...
        raise ActionTokenWorkflowError(
            "Hydra is unavailable in registered runtime"
        ) from exc
    environment = {
        "WAN_DIR": values["wan_dir"],
        "VIDEOX_HOME": values["videox_home"],
        "ACTION_TOKEN_VPM_SNAPSHOT": values["parent_snapshot"],
        "ACTION_TOKEN_TRAIN_CLIP_MANIFEST": values["train_manifest"],
        "ACTION_TOKEN_TRAIN_CACHE_METADATA": values["train_metadata"],
        "ACTION_TOKEN_VAL_CLIP_MANIFEST": values["validation_manifest"],
        "ACTION_TOKEN_VAL_CACHE_METADATA": values["validation_metadata"],
        "ACTION_TOKEN_STATS": values["stats"],
        "ACTION_TOKEN_STATS_SHA256": values["stats_sha256"],
        "ACTION_TOKEN_RUN_ROOT": values["training_root"],
    }
    ...
    if str(config.name) != arm.run_name:
        raise ActionTokenWorkflowError("composed arm config name differs")
    return contract
...
def _evaluation_argv(
    ...
    return [
        ...
        "--standalone",
        "--nproc_per_node=8",
        str(repo / "tools" / "action_token_evaluate.py"),
        "evaluate",
        "--registration",
...
def command_plan(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "required_environment": {
            "WAN_DIR": registration["runtime"]["wan_dir"],
            "VIDEOX_HOME": registration["runtime"]["videox_home"],
            "ACTION_TOKEN_VPM_SNAPSHOT_SHA256": screen.PARENT_SNAPSHOT_SHA256,
        ...
        payload["arms"].append(
            {
                ...
                "per_arm_environment": {
                    ...
                    "ACTION_TOKEN_TRAIN_CACHE_METADATA": values["train_metadata"],
                    "ACTION_TOKEN_VAL_CLIP_MANIFEST": values["validation_manifest"],
                    "ACTION_TOKEN_VAL_CACHE_METADATA": values[
                        "validation_metadata"
                    ],
                    "ACTION_TOKEN_STATS": values["stats"],
                    "ACTION_TOKEN_STATS_SHA256": values["stats_sha256"],
                    ...
                    "ACTION_TOKEN_REGISTRATION": str(
                        args.registration.resolve(strict=True)
                    ),
                    "ACTION_TOKEN_ARM_CODE": arm.code,
                },
            }
        )
...
def command_arm_values(args: argparse.Namespace) -> int:
    ...
    if args.format == "json":
        ...
        if any("\t" in values[key] or "\n" in values[key] for key in order):
            raise ActionTokenWorkflowError("arm value is unsafe for TSV")
...
def command_write_arm_plan(args: argparse.Namespace) -> int:
    ...
    for key in ("run_dir", "evaluation_dir"):
        ...
        if path.exists() or path.is_symlink():
            raise ActionTokenWorkflowError(f"fresh output exists: {path}")
...
def _read_runtime_verifier(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise ActionTokenWorkflowError("runtime verifier JSON is invalid") from exc
    if not isinstance(value, Mapping):
        raise ActionTokenWorkflowError("runtime verifier JSON is not a mapping")
    return dict(value)
...
def command_write_runtime_receipt(args: argparse.Namespace) -> int:
    ...
    if (
        ...
        raise ActionTokenWorkflowError("runtime receipt Slurm identity differs")
...
def command_wandb_check(_args: argparse.Namespace) -> int:
    ...
    if result.get("access") != "PRIVATE" or result.get("viewer_username") != "zijiandu":
        raise ActionTokenWorkflowError("W&B personal project privacy check failed")
...
def command_seal_wandb(args: argparse.Namespace) -> int:
    ...
    if not (run_dir / "training_complete.json").is_file():
        raise ActionTokenWorkflowError("training is incomplete before W&B sealing")
    output = run_dir / "wandb_run_complete.json"
    if output.exists() or output.is_symlink():
        raise ActionTokenWorkflowError("fresh W&B completion receipt exists")
    ...
    try:
        ...
        raise ActionTokenWorkflowError("W&B client is unavailable") from exc
    observed = None
    last_error = None
...
def command_analyze(args: argparse.Namespace) -> int:
    ...
    return analysis.command_analyze(
        argparse.Namespace(
            control_inventory=root / "evaluation" / "at-off" / "inventory.json",
            candidate_inventory=root / "evaluation" / "at-on" / "inventory.json",
