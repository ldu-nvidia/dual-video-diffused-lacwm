...
SBATCH_SCRIPT="$REPO_ROOT/tools/slurm/action_token.sbatch"
REGISTRATION=""
PYTHON_BIN=""
EXPECTED_COMMIT=""
PARTITION=batch
...
usage() {
  cat <<'EOF'
Usage: submit_action_token.sh --registration PATH --python PATH
...
JOB_NAME="action-token-$(basename "$OUTPUT_ROOT")"
...
if [[ -n "$(squeue -h -u "${USER:?}" -n "$JOB_NAME" -o '%A')" ]]; then
  die "an action-token job with this immutable name is already active"
fi
mkdir -m 700 "$LOG_DIR"
