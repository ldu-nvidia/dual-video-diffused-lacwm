...
HELPER="$REPO_ROOT/tools/dual_abc_causal_screen.py"
SBATCH_SCRIPT="$REPO_ROOT/tools/slurm/dual_abc_causal_screen.sbatch"
...
RUN_ROOT="$LACWM_BASE/runs/dual_video_diffusion/ztf_causal_screen"
...
usage() {
  cat <<'EOF'
Usage: tools/slurm/submit_dual_abc_causal_screen.sh [options]

Preview or submit the five-arm ABC causal TF screen as a 0-4 Slurm array:

  0  off,      fixed state scale 0.00
  1  matched,  fixed state scale 0.03
  2  shuffled, fixed state scale 0.03
  3  matched,  fixed state scale 0.10
  4  shuffled, fixed state scale 0.10
...
[[ "$MAX_CONCURRENT_ARMS" =~ ^[1-5]$ ]] || \
  die "--max-concurrent-arms must be an integer from 1 through 5"
...
if [[ -z "$SCREEN_ID" ]]; then
  SCREEN_ID="abc200-tf-causal-$(date -u +%Y%m%dT%H%M%SZ)-${EXPECTED_COMMIT:0:10}"
fi
...
JOB_NAME="dual-tf-causal-${SCREEN_ID:0:70}"
SBATCH_ARGS=(
  --parsable
  --nodes=1
  --ntasks=1
...
printf 'Validated causal-screen sbatch command:'
printf ' %q' "${COMMAND[@]}"
printf '\n'
...
JOB_ID="$("${COMMAND[@]}")" || die "Slurm rejected the causal-screen submission"
...
echo "Submitted causal-screen Slurm array: $JOB_ID"
