#!/usr/bin/env python3
"""Target-free val64 NFE1 evaluator for TFREG-OFF versus TFREG-ON."""
...
class TFREGEvaluationError(RuntimeError):
    pass
...
def _load_model_dataset(registration, seal, arm_code, device):
    ...
    if (
        ...
        raise TFREGEvaluationError("sealed snapshot metadata differs")
    incompatible = model.load_state_dict(snapshot["model"], strict=True)
    if incompatible.missing_keys or incompatible.unexpected_keys:
        raise TFREGEvaluationError(f"strict checkpoint load failed: {incompatible}")
    del snapshot
    if any(
        "spectral" in name.lower() or "tf_" in name.lower()
        for name, _ in model.named_parameters()
    ):
        raise TFREGEvaluationError("training-only treatment added inference parameters")
    model = model.to(device=device).eval()
    if getattr(model.forward_model.transformer, "teacache", None) is not None:
        raise TFREGEvaluationError("evaluation requires stateless Wan inference")
    ...
    if len(dataset) != contract.VALIDATION_CLIPS:
        raise TFREGEvaluationError("validation dataset is not val64")
    child = dataset.datasets["ABC"]
    if getattr(child, "_targets", None) is not None:
        raise TFREGEvaluationError("target-free dataset opened V-JEPA targets")
...
def _episode_rows(registration):
    ...
    if len(rows) != contract.VALIDATION_CLIPS:
        raise TFREGEvaluationError("val64 manifest cardinality changed")
    return rows
...
def _episode_disjoint_donor(actions, clip_index, episode_rows):
    ...
    if shift is None:
        raise TFREGEvaluationError("global batch lacks episode-disjoint action donors")
...
def _sample(model, history, actions, morphology, *, clip_index):
    ...
    if calls != 1 or sample.wan_calls != 1:
        raise TFREGEvaluationError("NFE1 sample did not execute exactly one Wan call")
    return sample
...
def command_evaluate(args: argparse.Namespace) -> int:
    ...
    if os.environ.get("WANDB_MODE") != "disabled":
        raise TFREGEvaluationError("evaluation requires WANDB_MODE=disabled")
    ...
    try:
        ...
        if args.output_dir != expected_output:
            raise TFREGEvaluationError(f"output must be {expected_output}")
        if rank == 0:
            if args.output_dir.exists() or args.output_dir.is_symlink():
                raise TFREGEvaluationError("evaluation output must be fresh")
        ...
        def forbidden_spectral_call(*_args, **_kwargs):
            raise TFREGEvaluationError(
                "training-only spectral loss ran during evaluation"
            )
        ...
        if getattr(child, "_targets", None) is not None:
            raise TFREGEvaluationError("evaluation opened a cached V-JEPA target")
        ...
        if rank == 0:
            ...
            if all_indexes != list(range(contract.VALIDATION_CLIPS)):
                raise TFREGEvaluationError("distributed val64 coverage differs")
            _write_rows(args.output_dir / "rows.jsonl", merged)
            inventory = contract.identity(
                {
                    "schema_version": 1,
                    "kind": "tf_training_only_evaluation_inventory",
...
def build_parser():
    ...
    parser.add_argument("--arm", choices=("TFREG-OFF", "TFREG-ON"), required=True)
