#!/usr/bin/env python3
"""Prospective contract, sealing, and paired analysis for TFREG.
...
SCHEMA_VERSION = 1
REGISTRATION_KIND = "tf_training_only_registration"
SEAL_KIND = "tf_training_only_post_training_seal"
RESULT_SCHEMA = "tf_training_only_paired_quality_v1"
TRAIN_UPDATES = 200
SEED = 1234
WORLD_SIZE = 8
TRAIN_CLIPS = 512
...
class TFREGContractError(RuntimeError):
    pass
...
ARMS = (
    Arm(
        "TFREG-OFF",
        "ravenhuang/wan-dit/tfreg_off",
        "tfreg-off-seed1234-u000200",
        0.0,
    ),
    Arm(
        "TFREG-ON",
        "ravenhuang/wan-dit/tfreg_on",
        "tfreg-on-seed1234-u000200",
        0.05,
    ),
)
...
def _regular_file(path: Path, label: str) -> Path:
    if not path.is_absolute() or not path.is_file() or path.is_symlink():
        raise TFREGContractError(f"{label} must be an absolute regular file")
    return path.resolve(strict=True)


def _directory(path: Path, label: str) -> Path:
    if not path.is_absolute() or not path.is_dir() or path.is_symlink():
        raise TFREGContractError(f"{label} must be an absolute directory")
    return path.resolve(strict=True)
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise TFREGContractError(f"invalid {label}") from exc
    if not isinstance(value, dict):
        raise TFREGContractError(f"{label} must contain one object")
    return value
...
def exclusive_json(path: Path, payload: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        raise TFREGContractError(f"refusing to overwrite {path}") from exc
...
def _git(repo: Path, *args: str) -> str:
    ...
    if result.returncode:
        raise TFREGContractError(result.stderr.strip())
    return result.stdout.strip()


def _source_record(expected_commit: str) -> dict[str, Any]:
    if COMMIT_RE.fullmatch(expected_commit) is None:
        raise TFREGContractError("expected source commit must be full SHA-1")
    if _git(REPO_ROOT, "rev-parse", "HEAD") != expected_commit:
        raise TFREGContractError("source HEAD differs from expected commit")
    if _git(REPO_ROOT, "status", "--porcelain", "--untracked-files=all"):
        raise TFREGContractError("source worktree must be clean")
...
def _runtime_record(python: Path, videox: Path, wan: Path) -> dict[str, Any]:
    if (
        ...
        raise TFREGContractError("Python launcher must be an absolute executable")
    ...
    if _git(videox, "rev-parse", "HEAD") != VIDEOX_COMMIT:
        raise TFREGContractError("VideoX commit differs")
    if _git(videox, "status", "--porcelain", "--untracked-files=all"):
        raise TFREGContractError("VideoX source must be clean")
    return {
        "python": {
            "launcher": str(python),
...
def _cache_file(metadata_path: Path, metadata: Mapping[str, Any], key: str) -> Path:
    ...
    if not isinstance(value, str) or not value:
        raise TFREGContractError(f"cache metadata lacks {key}")
    path = Path(value)
...
def _cache_record(manifest_path: Path, metadata_path: Path, *, split: str, count: int):
    ...
    if len(rows) != count:
        raise TFREGContractError(f"{split} clip count differs")
    for index, row in enumerate(rows):
        if (
            ...
            raise TFREGContractError(f"{split} manifest row {index} differs")
    if (
        ...
        raise TFREGContractError(f"{split} cache schema/identity differs")
    arrays = {}
    for key, digest_key in (
        ...
        if record["sha256"] != metadata.get(digest_key):
            raise TFREGContractError(f"{split} {key} digest differs")
        arrays[key] = record
    return {
        "split": split,
        "clip_count": count,
...
def _unchanged(record: Mapping[str, Any], label: str) -> None:
    ...
    if any(current[key] != record.get(key) for key in ("path", "bytes", "sha256")):
        raise TFREGContractError(f"registered {label} changed")
...
def _snapshot_record(path: Path):
    ...
    if record["sha256"] != PARENT_SNAPSHOT_SHA256:
        raise TFREGContractError("parent snapshot digest differs")
    snapshot = torch.load(
...
def _private_wandb() -> dict[str, Any]:
    ...
    if result.get("access") != "PRIVATE" or viewer != WANDB_ENTITY:
        raise TFREGContractError("W&B personal project is not private")
    return {
        "entity": WANDB_ENTITY,
...
def load_registration(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise TFREGContractError("registration identity/location differs")
    return value
...
def load_seal(registration: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise TFREGContractError("post-training seal differs")
    return value


def revalidate(registration: Mapping[str, Any], stage: str) -> dict[str, Any]:
    if stage not in {"train", "validation"}:
        raise TFREGContractError("stage must be train or validation")
    source = registration["source"]
    if _source_record(source["git_commit"]) != source:
        raise TFREGContractError("registered source changed")
...
def validate_resolved_config(config, registration, arm: Arm) -> None:
    ...
    spectral = model["spectral_regularization"]
    expected_exclusions = [
        "forward_model.tf_token_adapter",
        "forward_model.tf_clock_embedding",
        "forward_model.tf_velocity_head",
    ...
    if (
        model.get("_target_")
        != "lam.tf_training_only_model.TFTrainingOnlyExplicitActionDiT"
        or spectral
        != {
            "loss_weight": arm.loss_weight,
            "spatial_band_fraction": 0.5,
            "phase_weight": 0.25,
            "epsilon": 1.0e-4,
        }
        ...
        raise TFREGContractError(f"{arm.code} resolved config differs")
    for split, record in (
        ...
        if (
            ...
            raise TFREGContractError(f"{arm.code} {split} cache differs")


def validate_result_rows(rows, arm: Arm) -> None:
    expected = VALIDATION_CLIPS * len(ACTION_CONTROLS)
    if len(rows) != expected:
        raise TFREGContractError(f"{arm.code} result row count differs")
    keys = set()
    for row in rows:
        ...
        if (
            ...
            or row.get("spectral_loss_calls_at_inference") != 0
    ...
    if keys != expected_keys:
        raise TFREGContractError(f"{arm.code} evaluation grid differs")
    for clip in range(VALIDATION_CLIPS):
...
def command_register(args) -> int:
    if (
        ...
        raise TFREGContractError("study root must be an absolute path under /lustre")
    if args.study_root.exists() or args.study_root.is_symlink():
        raise TFREGContractError("study root must be fresh")
    ...
    payload = identity(
        {
            ...
            "design": {
                ...
                "only_intervention": "0.05 times training-only spectral endpoint loss",
                "predicted_clean_formula": "x0_hat = x_sigma - sigma * v_hat",
                "signal_axes": "Wan future latent [B,16,T=2,H=23,W=120]",
                "transform": "orthonormal rfftn over (T,H,W)",
                "band": "all temporal frequencies; |fy|,fx <= 0.25 cycles/sample (50% Nyquist)",
                "amplitude": "Smooth-L1 of target-RMS-normalized log1p magnitudes",
                "phase": "target-amplitude-weighted 1-cos cross-spectrum phase, global DC excluded",
                "phase_weight": 0.25,
...
def command_values(args) -> int:
    ...
    if args.format == "json":
        ...
        if any("\t" in str(values[key]) or "\n" in str(values[key]) for key in order):
            raise TFREGContractError("unsafe TSV value")
...
def command_plan(args) -> int:
    ...
    for key in ("run_dir", "snapshot", "plan", "evaluation_dir"):
        if Path(paths[key]).exists() or Path(paths[key]).is_symlink():
            raise TFREGContractError(f"fresh output exists: {paths[key]}")
    payload = identity(
        {
            "schema_version": 1,
            "kind": "tf_training_only_arm_plan",
            "registration_identity_sha256": registration["identity_sha256"],
            "arm": arm.code,
            "run_identity_sha256": arm_identity(registration, arm),
            "paths": paths,
            "training_input_revalidation": revalidation,
            "only_difference": "spectral loss weight 0.0 versus 0.05",
...
def _verified_canary(registration: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or canary.get("spectral_loss_calls") != 0
        ...
        raise TFREGContractError("deployment canary differs")
    return canary
...
def command_seal(args) -> int:
    ...
    for arm in ARMS:
        ...
        if (
            ...
            raise TFREGContractError(f"{arm.code} completion differs")
...
def command_analyze(args) -> int:
    ...
    for arm in ARMS:
        ...
        if (
            ...
            raise TFREGContractError(f"{arm.code} inventory differs")
        rows_by_arm[arm.code] = rows
    keyed = {
        arm: {(row["clip_index"], row["action_control"]): row for row in rows}
        for arm, rows in rows_by_arm.items()
    }
    for key in keyed["TFREG-OFF"]:
        off = keyed["TFREG-OFF"][key]
        on = keyed["TFREG-ON"][key]
        for field in (
            "noise_seed",
            "initial_noise_sha256",
            "action_tensor_sha256",
            ...
            if off[field] != on[field]:
                raise TFREGContractError(f"paired arm field differs: {field}")
    primary = {}
    gate = registration["decision_gate"]
    for metric in METRICS:
        off_values = [
            keyed["TFREG-OFF"][(clip, "aligned")][metric]
            for clip in range(VALIDATION_CLIPS)
        ]
        on_values = [
            keyed["TFREG-ON"][(clip, "aligned")][metric]
            for clip in range(VALIDATION_CLIPS)
        ]
    ...
    payload = identity(
        {
            "schema_version": 1,
            "kind": "tf_training_only_final_analysis",
            "analyzed_at": _now(),
            "registration_identity_sha256": registration["identity_sha256"],
            ...
            "interpretation": (
                "training-only spectrum improves deployable NFE1 generation"
                if overall
                else "training-only spectrum did not clear the preregistered deployable NFE1 gate"
            ),
            "spectral_or_auxiliary_inference_calls": 0,
            "protected_test_accessed": False,
        }
    )
