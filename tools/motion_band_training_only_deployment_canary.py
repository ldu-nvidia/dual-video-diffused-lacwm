#!/usr/bin/env python3
"""One-clip B200 proof that LFMREG has no motion-loss inference path."""
...
def _load_config(registration):
    ...
    with initialize_config_dir(version_base=None, config_dir=str(config_dir)):
        cfg = compose(
            config_name="train",
            overrides=[
                "+experiments_0908=ravenhuang/wan-dit/lfmreg_on",
            ],
        )
...
def command_run(args: argparse.Namespace) -> int:
    ...
    if registration.get("kind") != "motion_band_training_only_registration":
    ...
    inference_parameters = [
        ...
        if "motion_band" in name.lower() or "tf_" in name.lower()
    ]
    ...
    # If the training-only structural loss is reached, fail immediately.
    ...
    def forbidden_motion_band_call(*_args, **_kwargs):
        raise DeploymentCanaryError("motion training loss was called at inference")
    ...
    try:
        with (
            ...
            model.motion_band_loss_weight = 0.0
            ...
            model.motion_band_loss_weight = 0.05
    ...
    report = {
        "schema_version": 1,
        "kind": "motion_band_training_only_deployment_canary",
        "status": "passed",
