...
analysis. Validation and protected-test actions never contribute statistics.
...
these data/noise hashes differ between arms. Base/residual/augmented action
...
`group=null`, online mode, and registration-derived run identities.

## Deployable evaluation

Each of all 64 validation clips is sampled from identical deterministic noise.
...
endpoint in a local batch completes. The aligned endpoints use NFE `1,2,4`;
zero and shuffled attribution diagnostics use NFE `1`. A forward hook verifies
actual Wan calls. Preparation, Wan trajectory, decode, and total batch latency
are measured with CUDA synchronization.
...
nine fixed contrasts (`3 NFE x 3 metrics`) and action attribution has six
(`2 controls x 3 metrics`), each with its own one-sided Bonferroni family.
...
2. video-latent NMSE and decoded MSE point estimates and bounds are each above
   `-1%`;
3. within `AV-DELTA`, aligned actions beat both zero and episode-disjoint
   shuffled actions on temporal MSE by at least `+0.5%` in point estimate and
   bound, with the same `-1%` guardrails.
