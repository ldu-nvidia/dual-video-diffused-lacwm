...
For a valid archive, bounded NPY headers and central/local ZIP records are
parsed with exact, unbuffered `os.pread` calls.  The builder then uses exact
`os.pread` calls for only:
...
The 65 action rows are prospectively registered candidate commands available
to the deployable sampler, not future executed/measured state.  They are read
only to enforce exact equality with the already immutable action-cache row.
The per-row ledger therefore distinguishes 56 returned observed-state bytes
at frame 4 from 3,640 returned registered candidate-action bytes.  A runtime
invariant rejects any read intersecting unselected NPY array data.  The total
is 3,696 selected array-data bytes, `(14 + 65 * 14) * 4`; future measured-state,
unregistered-action, and unused-member-payload array-data counts must each be
exactly zero.
